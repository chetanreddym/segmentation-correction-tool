  ...
  extends PointOpImage
{
  ...
  protected void computeRect(Raster[] sources, WritableRaster dest, Rectangle destRect)
  {
    ...
    MediaLibAccessor alphaMA = new MediaLibAccessor(alpha.getData(destRect), destRect, formatTag);
    ...
    case 3: 
      for (int i = 0; i < dstMLI.length; i++) {
        Image.Blend(dstMLI[i], srcMLI1[i], srcMLI2[i], alphaMLI[0]);
      }
    ...
    case 5: 
      for (int i = 0; i < dstMLI.length; i++) {
        Image.Blend_Fp(dstMLI[i], srcMLI1[i], srcMLI2[i], alphaMLI[0]);
      }
