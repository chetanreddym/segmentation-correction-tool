  ...
  extends RandomIterCSM
{
  ...
  public final int getSample(int x, int y, int b) {
    ...
    return (int)bankData[b][((x - sampleModelTranslateX) * pixelStride + (y - sampleModelTranslateY) * scanlineStride + bandOffsets[b])];
  }
  ...
  {
    makeCurrent(x - boundsX, y - boundsX);
    return bankData[b][((x - sampleModelTranslateX) * pixelStride + (y - sampleModelTranslateY) * scanlineStride + bandOffsets[b])];
  }
