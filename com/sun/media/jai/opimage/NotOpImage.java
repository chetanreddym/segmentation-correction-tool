  ...
  protected void computeRect(Raster[] sources, WritableRaster dest, Rectangle destRect)
  {
    ...
    if (dst.isBinary()) {
      ...
      for (int i = 0; i < length; i++) {
        dstBits[i] = ((byte)(srcBits[i] ^ 0xFFFFFFFF));
      }
    ...
    {
    case 0: 
      byteLoop(dstNumBands, dstWidth, dstHeight, srcLineStride, srcPixelStride, srcBandOffsets, src.getByteDataArrays(), dstLineStride, dstPixelStride, dstBandOffsets, dst.getByteDataArrays());
    ...
    case 1: 
    case 2: 
      shortLoop(dstNumBands, dstWidth, dstHeight, srcLineStride, srcPixelStride, srcBandOffsets, src.getShortDataArrays(), dstLineStride, dstPixelStride, dstBandOffsets, dst.getShortDataArrays());
    ...
    case 3: 
      intLoop(dstNumBands, dstWidth, dstHeight, srcLineStride, srcPixelStride, srcBandOffsets, src.getIntDataArrays(), dstLineStride, dstPixelStride, dstBandOffsets, dst.getIntDataArrays());
    }
    ...
    for (int b = 0; b < dstNumBands; b++) {
      ...
      for (int h = 0; h < dstHeight; h++) {
        ...
        for (int w = 0; w < dstWidth; w++) {
          d[dstPixelOffset] = ((byte)(s[srcPixelOffset] ^ 0xFFFFFFFF));
  ...
  private void shortLoop(int dstNumBands, int dstWidth, int dstHeight, int srcLineStride, int srcPixelStride, int[] srcBandOffsets, short[][] srcData, int dstLineStride, int dstPixelStride, int[] dstBandOffsets, short[][] dstData)
  {
    for (int b = 0; b < dstNumBands; b++) {
      ...
      for (int h = 0; h < dstHeight; h++) {
        ...
        for (int w = 0; w < dstWidth; w++) {
          d[dstPixelOffset] = ((short)(s[srcPixelOffset] ^ 0xFFFFFFFF));
          srcPixelOffset += srcPixelStride;
          dstPixelOffset += dstPixelStride;
        }
      }
