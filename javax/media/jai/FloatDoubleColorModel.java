  ...
  extends ComponentColorModel
{
  ...
  public FloatDoubleColorModel(ColorSpace colorSpace, boolean hasAlpha, boolean isAlphaPremultiplied, int transparency, int transferType)
  {
    ...
    numComponents = (this.numColorComponents = colorSpace.getNumComponents());
  ...
  private int getSample(Object inData, int sample) {
    ...
    if (transferType == 4) {
      float[] fdata = (float[])inData;
      float[] rgb; float[] rgb; if (needAlpha) {
        float falp = fdata[numColorComponents];
        if (falp == 0.0D) return 0;
    ...
    return (int)(rgb[sample] * 255.0F + 0.5D);
  }
  ...
  public int getAlpha(Object inData)
  {
    ...
    double[] ddata = (double[])inData;
    return (int)(ddata[numColorComponents] * 255.0D + 0.5D);
  }
  ...
  public Object getDataElements(int rgb, Object pixel)
  {
    if (transferType == 4) {
      ...
      if (colorSpace.isCS_sRGB()) {
        ...
        float gray = (rgb >> 16 & 0xFF) * (0.299F * inv255) + (rgb >> 8 & 0xFF) * (0.587F * inv255) + (rgb & 0xFF) * (0.114F * inv255);
    ...
    if (colorSpace.isCS_sRGB()) {
      ...
      double gray = (rgb >> 16 & 0xFF) * (0.299D * inv255) + (rgb >> 8 & 0xFF) * (0.587D * inv255) + (rgb & 0xFF) * (0.114D * inv255);
